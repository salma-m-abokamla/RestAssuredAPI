...
public class Discover {
	...
	public static void Discover(String baseURL, String subscription) {
		...
		String response = given().headers(ReUsableMethods.generalHeaders(subscription))
				...
				.header("JWT", SoftToken.backendJwtSoftToken).when().get(discoverResoureURL).then().assertThat()
		...
		VSTSFileUploader.addResponseContentToUploadableFile(response, MVA10APIS.DISCOVER.getName());
