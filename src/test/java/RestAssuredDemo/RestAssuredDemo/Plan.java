...
public class Plan {
	...
	public static void Plan(String baseURL, String subscription) {
		...
		String response = given().headers(ReUsableMethods.generalHeaders(subscription))
				...
				.header("JWT", SoftToken.backendJwtSoftToken).when().get(planResoureURL).then().assertThat()
		...
		VSTSFileUploader.addResponseContentToUploadableFile(response, MVA10APIS.PLAN.getName());
