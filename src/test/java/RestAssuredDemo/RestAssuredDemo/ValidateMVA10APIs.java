...
public class ValidateMVA10APIs extends BaseClass {
	...
	public void GetAccounts(String baseURL, String accountsResoureURL, String subscription) {
		...
		String responce = given().log().all().headers(ReUsableMethods.generalHeaders(subscription))
				...
				.asString();
		JsonPath js1 = ReUsableMethods.rawToJson(responce);
		accountType = js1.getString("accountType");
		accountIdHash = js1.getString("accountIdHash");
	...
	public void GetSubscriptionsList(String baseURL, String accountsResoureURL, String subscription) {
		...
		String responce = given().log().all().headers(ReUsableMethods.generalHeaders(subscription))
				.header("Segment", segment).header("Content-type", "application/json")
				.header("Subscription-Type", subscriptionType).header("JWT", backendJwtSoftToken)
				.header("Root-Full-Access-Token", fullAccessToken).header("Complex-Subscription", "false")
				.header("accountId", accountId).header("Full-Access-Token", fullAccessToken)
				.header("Parent-Subscription", subscription).header("accountType", accountType)
				.header("accountIdHash", accountIdHash).when()
				.get(accountsResoureURL.concat("/").concat(accountId).concat("/subscriptions")).then().log().all()
				.assertThat().statusCode(200).extract().response().asString();
		;
